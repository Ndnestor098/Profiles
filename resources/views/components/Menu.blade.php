<aside class="w-16 bg-blue-600 text-white flex flex-col items-center py-4 gap-4 shadow-lg">
    ...
    <nav class="flex flex-col gap-3 mt-2">
        ...
        <a href="{{ route('profiles.index') }}"
           class="w-10 h-10 rounded-2xl flex items-center justify-center {{ Route::is('profiles.index') ? 'bg-white/20' : 'hover:bg-white/10' }}">
        ...
        {{-- Folder --}}
        <a href="{{ route('profiles.upload') }}"
           class="w-10 h-10 rounded-2xl flex items-center justify-center {{ Route::is('profiles.upload') ? 'bg-white/20' : 'hover:bg-white/10' }}">
            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none">
